...
// It should have been uploaded with the FS, but if it is missing, create a
// blank copy of it
...
bool loadConfig(networkConfig* config) {
  ...
  if (strlen(doc["alias"]) == 0) {
    strlcpy(config->alias, "aq-monitor-main", sizeof(config->alias));
  } else {
    strlcpy(config->alias, doc["alias"], sizeof(config->alias));
  }
  strlcpy(tempConfig.alias, config->alias, sizeof(tempConfig.alias));
